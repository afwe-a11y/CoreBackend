...
public class ProductRepositoryImpl implements ProductRepositoryPort {
  ...
  public ProductRepositoryImpl(ProductMapper productMapper, JsonCodec jsonCodec) {
    this.productMapper = productMapper;
    this.jsonCodec = jsonCodec;
  }

  @Override
  public ProductPortData save(ProductPortData product) {
  ...
  @Override
  public ProductPortData update(ProductPortData product) {
    ProductPo po = toPo(product);
  ...
  @Override
  public ProductPortData findById(Long id) {
    return toModel(productMapper.findById(id));
  }

  @Override
  public ProductPortData findByProductKey(String productKey) {
    return toModel(productMapper.findByProductKey(productKey));
  }

  @Override
  public List<ProductPortData> listAll() {
  ...
  @Override
  public List<ProductPortData> listByDeviceModelId(Long deviceModelId) {
    return productMapper.listByDeviceModelId(deviceModelId).stream()
        .map(this::toModel)
        .collect(Collectors.toList());
  }

  private ProductPortData toModel(ProductPo po) {
    if (po == null) {
      return null;
    }
  ...
  private ProductPo toPo(ProductPortData product) {
